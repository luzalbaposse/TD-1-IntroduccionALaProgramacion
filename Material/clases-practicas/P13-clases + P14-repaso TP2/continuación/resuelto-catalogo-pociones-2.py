...
class CatalogoPociones:
    ...
    def listar_pociones_con_n_ingredientes(self, n:int) -> List[Tuple[str,Pocion]]:
        ...
        for dif in self.pociones_dificultad:
            pociones = self.listar_por_dificultad[dif]
            for p in pociones:
    ...
    def tienen_caracteristica(self, dif:str, caracteristica:str)-> List[Pocion]:
        ...
        Requiere: nada
        Devuelve: una lista de Pociones, donde dif
        refiere a una cierta dificultad y caracteristica a una cierta caracteristica
        que se corresponde con la dificultad y caracteristica de las pociones que
        se deben guardar en la lista de salida,
        Por ejemplo, si el método se invoca con los argumentos 'Advanced' y 'Spiralling steam', 
        la lista contendrá pociones de tipo Advance y con esa característica.
        Además, en la lista las pociones aparecen
        ordenadas de acuerdo al orden definido en la clase Pocion.
        '''
        ...
        for pocion in self.pociones_dificultad[dif]:
            if caracteristica in pocion.caracteristicas:
                pociones_con_caracterisitica.append(pocion)
                
        return pociones_con_caracterisitica
    
    def cantidad(self, dif:str)-> int: 
        '''
        Requiere: nada
        Devuelve: la cantidad de pociones que son de dificultad dif.
        '''      
        return len(self.pociones_dificultad[dif])
    
    def ingredientes (self, dif:str)-> Set[str]:
        '''
        Requiere: nada
        Devuelve: el conjunto de ingredientes que se usan para preparar todas las pociones que sean 
        de dificultad dif.
        '''  
        ingredientes:Set[str] = set()
        for pocion in self.pociones_dificultad[dif]:
            ingredientes = ingredientes | set(pocion.ingredientes_conocidos)
